...
public class Block {
	...
	public boolean check(String nonce2, String en2) {
		...
		//System.out.println(hexlify(concatByteArray(header_prefix_bin, nonce_bin)));
		//System.out.println(hexlify(sha256(concatByteArray(header_prefix_bin, nonce_bin))));
		//System.out.println(hexlify(sha256d(concatByteArray(header_prefix_bin, nonce_bin))));
		System.out.println(hash);
		//System.out.println(Target());
		//System.out.println(hash.compareTo(Target()) < 0);
		return hash.compareTo(Target()) < 0;
	}
	
	public void printByteArray(byte[] ba) {
		StringBuilder pba = new StringBuilder();
		for(int i = 0; i < ba.length; ++i) pba.append("," + (int)ba[i]);
		System.out.println(pba.toString());
	}
